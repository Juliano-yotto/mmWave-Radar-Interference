...
%% FMCW干扰信号参数设置
R_int_FMCW = [1.5 20 10];
V_int_FMCW = [1.5 8 2];
N_int_FMCW = length(R_int_FMCW);

N_int_chirp = 1;                             % 一个FMCW干扰跨越的发射chirp周期数
...
for int_FMCW = 0:1:N_int_division-1
    ...
    S_int_FMCW = AT*exp(1i*2*pi*(fint_FMCW*(t_int-int_FMCW*T_int)+slope_int/2*t_int.^2)); %FMCW干扰信号
...
for i = 0:1:N_chirp-1  %chirp周期循环
    ...
    for n_cw = 1:1:N_int_CW
        ...
        td_int_CW{n_cw,1} = distant_int_CW{n_cw,1}/c;                              %干扰时延更新   *注意这里时延不是两倍了
    end
